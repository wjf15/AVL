function [status,results, foldName] = createSTOut(avlLoc,RUNNAME,planefile, runfile, RunFile, runCount,makefolder)
    %Formats
    ...
    fprintf(TextFile,'%s\n',    'f');
    fprintf(TextFile,'%s\n',    runfile);
    foldName = ['C:\Users\User\Documents\Fall2021\402A\AVL\STfiles\' char(RUNNAME)];
    ...
    for i=1:runCount
        ...
        fprintf(TextFile,'%s\n',    'x');
        fprintf(TextFile,'%s\n',    'st');
        title = [foldName '\run' num2str(i)  '.st'];
        fprintf(TextFile,'%s\n',    title);
    end
